...
- I intentionally kept the application logic simple to focus on **infrastructure behaviour** rather than frontend complexity.
- Redis was chosen as a dependency to surface **state, persistence, and networking issues** early in the learning process.
- Docker Compose was used instead of Kubernetes to keep system complexity **proportional to scope**, while still modelling real multi-service architectures.
- Environment variables were introduced to decouple configuration from images and support **production-style deployments**.
- NGINX was added only after scaling exposed real limitations, rather than pre-emptively introducing unnecessary components.

These decisions were made deliberately to prioritise **clarity, debuggability, and realistic DevOps workflows** over feature count.
